...
- certain research questions involve continuous outcomes [i.e: tumor size, blood pressure, and symptom severity scores] 
- researchers aim to determine whether an observed mean or difference in means is large enough to reflect a true population effect rather than random sampling variability.

**Lecture Concepts**:

- when to use hypothesis tests for continuous outcomes
- one-sample, independent two-sample, and paired t-tests
- t-distribution and why it is used
- t-statistics and p-values for evaluating evidence against the null hypothesis
...
- dach row represents one patient in a clinical oncology study
- `tumor_reduction` : percent change in tumor size after treatment for that patient
- positive values indicate tumor shrinkage (reduction in size)
- dataset is a sample drawn from a larger population of similar patients
- Goal: use the sample to estimate the true mean tumor reduction (μ)
- hypothesis test evaluates whether the population mean tumor reduction is greater
...
- alternative hypothesis is $H_A:μ>0$ , the p-value is calculated using the right tail of the t distribution
...
![](images/right_tailed_dist.png){width=65% fig-align="center"}
...
- dataset is a sample from a larger population of patients who could receive either treatment
- goal: compare the average systolic blood pressure between Treatment A and Treatment B
- statistical question: does one treatment lead to lower average systolic blood pressure in the population?
- each row represents one patient in a blood pressure clinical study
- `treatment` indicates which group the patient was assigned to [Treatment A or Treatment B]
- `systolic_bp` : patient’s measured systolic blood pressure after treatment
- Treatment A and Treatment B each contain 20 patients (balanced sample design)
...
- dataset is structured as paired observations (each patient measured twice)
- goal: assess whether symptoms differ after treatment on average
- statistical question: is the mean difference (before − after) different than 0 in the
- each row represents one patient in a pre–post clinical study
- `patient_id` identifies each individual patient in the sample
- `symptom_before` : patient’s symptom severity score before treatment
- `symptom_after` : patient’s symptom severity score after treatment
